    ...
    performanceTracker = function ($rootScope, $log, $timeout, $interval, $http, proxyManager, storage, timeUtils, domainUtils, ROLES, LOG_URL, VER) {
        ...
        applyTestResult = function (result) {
            ...
            if (result.type === 'speed') {
                speed = result.contentLength / 1000 / (result.timeEnded - result.timeHeadersReceived);
                if (!result.error) {
                ...
                latency = 1000 * (result.timeHeadersReceived - result.timeSendHeaders);
                if (!result.error) {
                    reportPerf(result.proxy, {
                        latency: latency
                    })
        ...
        onSendHeaders = function (details) {
            ...
            result.timeSendHeaders = parseInt(details.timeStamp) / 1000;
        ...
        onHeadersReceived = function (details) {
            ...
            if (result) {
                ...
                result.timeHeadersReceived = parseInt(details.timeStamp) / 1000;
        ...
        onErrorOccurred = function (details) {
            ...
            result = perfResults[details.requestId];
            if (result) {
                result.timeEnded = parseInt(details.timeStamp) / 1000;
        ...
        onProxyError = function (details) {
            ...
            return chrome.proxy.settings.get({}, function (settings) {
                return chrome.management.getAll(function (exts) {
                    ...
                    script = (_ref1 = settings.value) != null ? (_ref2 = _ref1.pacScript) != null ? _ref2.data.slice(-1000) : void 0 : void 0;
